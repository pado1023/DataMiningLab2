...
public final class Lab1 {
    ...
    public static HashMap<String,Integer> tokenizeArticle(Elements topic, Elements places, Elements body){
    	// Tracks if the term has already appeared in this article
    ...
    public static Map<String, Map<String, Double>> convertToTf_Idf(Map<String, Map<String, Integer>> tf_idf_map){
        ...
        for (Map.Entry<String,Map<String, Integer>> vector: tf_idf_map.entrySet()){
        	...
        	for(Map.Entry<String, Integer> valueVector: valueMap.entrySet()){
        		...
        		if (tf_idf>0.015 || keyPair.contains("*") || keyPair.contains("{")){
    ...
    public static Map<String, Map<String, Integer>> filter(Map<String, Map<String, Integer>> heavy_filter){
    	...
    	for(Map.Entry<String, Map<String, Integer>> vector: heavy_filter.entrySet()){
    		...
    		for(Map.Entry<String, Integer> valueVector: single_map.entrySet()){
    			...
    			//Keep the topics and places organized
    			if(key.contains("{") || key.contains("[")){
    				temp_map.put(valueVector.getKey(), valueVector.getValue());
    			}else if(!containNumber(key)  && !key.equals("place") && !key.equals("reuter") && !key.contains("\\") && key.length() > 2 && termFreq.get(key) > 100){
    				temp_map.put(valueVector.getKey(), valueVector.getValue());
    			}
     ...
     * 
     */
    public static Map<String, Map<String, Integer>> markTopicPlaces(){
    	Map<String, Map<String, Integer>> map = new HashMap<String, Map<String, Integer>>();
    	for(Map.Entry<String, Map<String,Integer >> fv: tf_idf_featureMap.entrySet()){
    		Map<String, Integer> vector = fv.getValue();
    		Map<String, Integer> temp = new HashMap<String, Integer>();
    		
    		for(Map.Entry<String, Integer> v: vector.entrySet()){
    			boolean inTopics = false;
    			boolean inPlaces = false;
    			String[] top = topics_map.get(fv.getKey());
    			String[] plac =places_map.get(fv.getKey());
    			for(int i = 0; i < top.length; i++){
    				if(v.getKey().equals(top[i])){
    					inTopics = true;
    				}
    			}
    			
    			for(int i = 0; i < plac.length; i++){
    				if(v.getKey().equals(plac[i])){
    					inPlaces = true;
    				}
    			}
    			...
    			if(inTopics){
    				//Surround topics with {}
    				String x = "{"+v.getKey()+ "}";
    				temp.put(x, v.getValue());
    				
    			}else if(inPlaces){
    				String x = "["+v.getKey()+"]";
    				temp.put(x, v.getValue());
    			} else {
    				temp.put(v.getKey(), v.getValue());
    			}
    			
    			
    		}
    		map.put(fv.getKey(), temp);
    	}
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        for ( File fileEntry : folder.listFiles()) {
            ...
            Scanner fileScanner = new Scanner(new File("data/" + fileName), "UTF-8");
      ...
      Map<String,Map<String, Integer>> markedMap = markTopicPlaces();
      ...
      Map<String, Map<String, Integer>> updated_heavy_filtered_map = filter(markedMap);
       
      
       //Print the feature map/vectors
       //tf-idf printing
       
       ArrayList<String> corpusTerms = new ArrayList<String>();
     
       for (Map.Entry<String,Map<String,Double>> fv: updated_tf_idf_FeatureMap.entrySet()){
    	   for (Map.Entry<String, Double> articleFreq: fv.getValue().entrySet()){
    		   if (!corpusTerms.contains(articleFreq.getKey())){
    			   corpusTerms.add(articleFreq.getKey());
    		   }
    	   }
       }
       ...
       for (int x = 1; x<=numOfDoc; x+=1000){
    	   CsvWriter tf_idf_writer = new CsvWriter("tf-idf part "+ (x/1000+1) + ".csv");
    	   ...
    	   for (int y = x; y < x+1000; y++){
        	   ...
        	   System.out.println("Article "+y+" printed");
    	   }
    	   tf_idf_writer.close();
       }
       ...
       corpusTerms.clear();
       for (Map.Entry<String,Map<String,Integer>> fv: updated_heavy_filtered_map.entrySet()){
    	   for (Map.Entry<String, Integer> articleFreq: fv.getValue().entrySet()){
    		   if (!corpusTerms.contains(articleFreq.getKey())){
    			   corpusTerms.add(articleFreq.getKey());
    		   }
    	   }
       ...
       for (int x = 1; x<=numOfDoc; x+=1000){
    	   CsvWriter filter_writer = new CsvWriter("heavy-filtered-part "+ (x/1000+1) + ".csv");
    	   printCorpusTerms(corpusTerms,filter_writer);
    	   for (int y = x; y<x+1000; y++){
    		   if (y==numOfDoc) break;
        	   ...
        	   System.out.println("Filtered Article "+y+" printed");
    	   }
    	   filter_writer.close();
       }
